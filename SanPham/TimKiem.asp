...
<table width="100%" class="div">
  <tr>
    ...
    <td width="86%" valign="top">
        <br><br>
            <font size="+2">Kết quả tìm được</font>
        </div>
        <br>
			<%
                ...
                //tạo câu lệnh SQL
                sSQL = "SELECT TenSP, FORMAT(GiaSP, '#,##0') AS GiaSP, AnhSP FROM ThongTinSP WHERE TenSP LIKE N'%"+ TimKiem +"%' ORDER BY ThoiGianThem DESC";
                //thực thi câu lệnh SQL
                rs.Open(sSQL,cnn);
				if(rs.EOF || TimKiem == ''){
					Response.Write("<h2>Không tìm được sản phẩm. Vui lòng thử lại!!!</h2>");}
					else{
		   %>
            <%
                var index1=0;
