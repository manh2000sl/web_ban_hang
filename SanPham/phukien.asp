  ...
  <tr>
    <td width="14%" valign="top">
    	<div>
            ...
            <ul class="ul">
                ...
                <li class="hoatdong"><i class="iconpk"></i> Phụ kiện</li>
            </ul>
		</div>
	</td>
    ...
    <br><br>
        <%
                ...
                //tạo câu lệnh SQL
                SoLuong = "SELECT COUNT(TenSP) AS SoLuong FROM ThongTinSP WHERE LoaiSP='PK'";
                //thực thi câu lệnh SQL
                rs.Open(SoLuong,cnnSL);
				var SoLuong = rs.Fields.Item("SoLuong").Value;
		%>
            <font size="+2">PHỤ KIỆN (<%=SoLuong%> Sản phẩm)</font>
        <br>
			<%
                //tạo đối tượng Connection
                ...
                rs=Server.CreateObject("ADODB.Recordset");
                //tạo câu lệnh SQL
                sSQL = "SELECT TenSP, FORMAT(GiaSP, '#,##0') AS GiaSP, AnhSP FROM ThongTinSP WHERE LoaiSP='PK' ORDER BY ThoiGianThem DESC";
                //thực thi câu lệnh SQL
                rs.Open(sSQL,cnn);
