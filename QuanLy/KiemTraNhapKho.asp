...
<%
    ...
    SLNhap = Request("SLNhap");
    GiaNhap = Request("GiaNhap");
    cnnKho = Server.CreateObject("ADODB.Connection");
    cnnKho.Open(cStr);
	Kho = "INSERT INTO Kho VALUES(N'" + TenSP + "',CONVERT(VARCHAR(20), GETDATE(), 113),'" + SLNhap + "','" + GiaNhap + "','0', CONVERT(VARCHAR(20), GETDATE(), 113),'"+SLNhap+"')";
	cnnKho.Execute(Kho);
%>
    <script>
        alert("Bạn đã nhập sản phẩm vào kho thành công.");
