...
<%
    ...
    SP = "SELECT TenNSX FROM NhaSanXuat WHERE TenNSX = '"+TenNSX+"'";
    rs.Open(SP, cnn);
    if(rs.EOF){
	ThemNSX = "INSERT INTO NhaSanXuat VALUES('"+TenNSX+"','"+Logo+"',N'"+Website+"','"+SDT+"','"+Email+"', N'"+DiaChi+"')";
	cnn.Execute(ThemNSX);
%>
    <script>
        alert("Bạn đã thêm nhà sản xuất thành công.");
...
%>
    <script>
        alert("Tên nhà sản xuất đã được sử dụng, bạn vui lòng chọn tên khác...");
        document.location = "ThemNSX.asp";
    </script>
<% } %>
<!--Footer-->
