...
<table class="div" width="100%">
	<tr>
        ...
        <td width="60%" valign="top">
            ...
            <%
                var NDTraLoi = Request("NDTraLoi");
                MaPH = Request("MaPH");
	            cnnGui=Server.CreateObject("ADODB.Connection");
	            cnnGui.Open(cStr);
	            GuiThongBao = "UPDATE PhanHoiTV SET ThoiGianTL = CONVERT(VARCHAR(20), GETDATE(), 113), NDTraLoi = N'"+NDTraLoi+"', TrangThai='CHUADOC' WHERE MaPH = '"+MaPH+"'";
	            cnnGui.Execute(GuiThongBao);
            %>
            <script>
                document.location = "PhanHoi.asp";
                alert("Bạn đã trả lời thành công phản hồi mã <%=MaPH%>");
            </script>
        </td>
    </tr>
</table>
