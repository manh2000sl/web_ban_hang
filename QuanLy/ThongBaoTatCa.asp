...
<table class="div" width="100%">
	<tr>
        ...
        <td width="60%" valign="top">
            ...
            <%
                var NDTraLoi = Request("NoiDungMoi");
                    ChuDeMoi = Request("ChuDeMoi");
	            cnnGui=Server.CreateObject("ADODB.Connection");
	            cnnGui.Open(cStr);
                rs=Server.CreateObject("ADODB.Recordset");
				TaiKhoan = "SELECT TenDangNhap,HoatDong FROM ThanhVien WHERE HoatDong = 'YES'";
				rs.Open(TaiKhoan, cnnGui);  
				var index1=0;
				rs.MoveFirst();
				while (!rs.EOF) {
				    var TenTV = rs.Fields.Item("TenDangNhap").Value;
	                GuiThongBao = "INSERT INTO PhanHoiTV VALUES('"+TenTV+"', N'" + ChuDeMoi + "', '', N'', CONVERT(VARCHAR(20), GETDATE(), 113), N'" +NDTraLoi+ "', 'CHUADOC')";
	                cnnGui.Execute(GuiThongBao);
                    rs.MoveNext();
                    }
            %>
            <script>
                document.location = "PhanHoi.asp";
                alert("Bạn đã thông báo thành công đến tất cả thành viên.");
            </script>
        </td>
    </tr>
</table>
